...
#define RGBLedBulb_THERMAL_2530_COEFFICIET  2
#define RGBLedBulb_THERMAL_2530_OFFSET      475
#endif
...
void hwThermal_Monitor( bool enable )
{
  ...
  if ( enable == FALSE )
  {
    ...
    if ( zclRGBLedBulb_OnOff == LIGHT_ON )
    {
      hwLight_UpdateOnOff( LIGHT_ON );
    }
    osal_stop_timerEx( lampAppTaskID, RGBLedBulb_THERMAL_SAMPLE_EVT );
    return;
  }

  if ( currTemp > RGBLedBulb_THERMAL_THRESHOLD )
  {
    hwLight_UpdateOnOff( LIGHT_OFF );
    thermalShutdown = TRUE;
  }
  else if ( thermalShutdown && ( currTemp < RGBLedBulb_THERMAL_THRESHOLD ) )
  {
    thermalShutdown = FALSE;
    if ( zclRGBLedBulb_OnOff == LIGHT_ON )
    {
  ...
  osal_start_timerEx( lampAppTaskID, RGBLedBulb_THERMAL_SAMPLE_EVT,
                      RGBLedBulb_THERMAL_SAMPLE_INTERVAL );
}
...
static uint16 hwThermal_tempMeasure( void )
{
  ...
  TR0 &= ~0x01;
  ATEST &= ~0x01;
  tempSample = (tempSample - RGBLedBulb_THERMAL_2530_OFFSET)/RGBLedBulb_THERMAL_2530_COEFFICIET;
#if 0 // Debug
#if (HAL_LCD == TRUE)
