...
void zllEffects_InitiateEffect( uint16 clusterID, uint8 commandID, uint8 effectID, uint8 effectVariant )
{
  ...
  {
    osal_set_event( zclLight_TaskID, RGBLedBulb_EFFECT_PROCESS_EVT );
  }
}
    ...
    case EFFECT_ID_BLINK:
      {
        if ( _currentEffect.state == 0xFF )
        {
          ...
          hwLight_UpdateOnOff ( ( zclRGBLedBulb_OnOff == LIGHT_ON ) ? LIGHT_OFF : LIGHT_ON );
#else
          HalLedBlink ( HAL_LED_4, 0xFF, HAL_LED_DEFAULT_DUTY_CYCLE, HAL_LED_DEFAULT_FLASH_TIME );
#endif
          osal_start_timerEx( zclLight_TaskID, RGBLedBulb_EFFECT_PROCESS_EVT, 200 );
        }
      }
      break;
    ...
    case EFFECT_ID_BREATHE:
      {
        ...
        osal_start_timerEx( zclLight_TaskID, RGBLedBulb_EFFECT_PROCESS_EVT, 1000 );
      }
      break;
    ...
    case EFFECT_ID_OKAY:
      {
        ...
        hwLight_UpdateOnOff( ( _currentEffect.state & 0x01 ) ? LIGHT_ON : LIGHT_OFF );
        osal_start_timerEx( zclLight_TaskID, RGBLedBulb_EFFECT_PROCESS_EVT, 500 );
#endif
      }
      break;
    ...
    case EFFECT_ID_CHANNEL_CHANGE:
      {
        ...
        osal_start_timerEx( zclLight_TaskID, RGBLedBulb_EFFECT_PROCESS_EVT, 8000 );
#endif //ZLL_HW_LED_LAMP
#else //ZCL_COLOR_CTRL
        if ( _currentEffect.state == 0xFF )
        {
...
#endif
          osal_start_timerEx( zclLight_TaskID, RGBLedBulb_EFFECT_PROCESS_EVT, 7500 );
        }
#endif
      }
      break;
...
#endif //ZLL_HW_LED_LAMP
    osal_start_timerEx( zclLight_TaskID, RGBLedBulb_EFFECT_PROCESS_EVT, 100 );
#endif //ZCL_LEVEL_CTRL
  }
}
